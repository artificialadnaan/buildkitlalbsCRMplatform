...
/**
 * Seeds demo data for the given user. Skips if data already exists.
...
export async function seedDemoData(userId: string): Promise<boolean> {
  ...
  const existingCompanies = await db
    ...
    .from(companies)
    .where(eq(companies.assignedTo, userId));
  ...
  if (!localPipeline || !constructionPipeline) {
    console.error('Demo seed: pipelines not found — run db:seed first');
    return false;
  }
  ...
  const insertedDeals: Array<{ id: string; companyId: string }> = [];
  ...
  for (let i = 0; i < insertedDeals.length; i++) {
    ...
    if (d.status === 'won') {
      eventEntries.push({ dealId: insertedDeals[i].id, type: 'status_change', fromValue: 'open', toValue: 'won', createdAt: daysAgo(d.daysAgo - 10) });
    } else if (d.status === 'lost') {
      eventEntries.push({ dealId: insertedDeals[i].id, type: 'status_change', fromValue: 'open', toValue: 'lost', createdAt: daysAgo(d.daysAgo - 10) });
    }
  }
  ...
  if (activityEntries.length > 0) {
    await db.insert(activities).values(
      activityEntries.map(a => ({
        dealId: a.dealId, contactId: a.contactId, userId, type: a.type,
        subject: a.subject, body: a.body, createdAt: a.createdAt,
      }))
    );
  }
  ...
  const insertedProjects: Array<{ id: string; companyId: string }> = [];
  ...
  for (const t of taskDefs) {
    await db.insert(tasks).values({
      ...
      source: 'manual', dueDate: dateStr(t.dueOffset),
    });
  }

  // ── Time Entries ─────────────────────────────────────────────
  const timeEntryDefs = [
    { projectIdx: 0, desc: 'Fleet GPS API - endpoint design and implementation', minutes: 180, daysAgo: 10 },
    { projectIdx: 0, desc: 'Vehicle dashboard - React component build', minutes: 240, daysAgo: 8 },
    { projectIdx: 0, desc: 'Database schema design for fleet tracking', minutes: 120, daysAgo: 12 },
    { projectIdx: 0, desc: 'Client meeting - progress review', minutes: 45, daysAgo: 7, billable: false },
    { projectIdx: 0, desc: 'Maintenance scheduling - backend logic', minutes: 210, daysAgo: 5 },
    { projectIdx: 0, desc: 'Bug fix: GPS coordinate parsing', minutes: 60, daysAgo: 3 },
    { projectIdx: 0, desc: 'Code review and refactoring', minutes: 90, daysAgo: 2 },
    { projectIdx: 1, desc: 'Discovery interview with Dr. Sharma', minutes: 60, daysAgo: 30, billable: false },
    { projectIdx: 1, desc: 'Wireframe creation - 8 pages', minutes: 180, daysAgo: 20 },
    { projectIdx: 1, desc: 'Homepage design iteration', minutes: 150, daysAgo: 6 },
    { projectIdx: 1, desc: 'Service pages layout', minutes: 120, daysAgo: 4 },
    { projectIdx: 1, desc: 'Client revision review', minutes: 45, daysAgo: 2, billable: false },
    { projectIdx: 2, desc: 'SEO keyword research and strategy', minutes: 120, daysAgo: 45 },
    { projectIdx: 2, desc: 'Full site development - 12 pages', minutes: 480, daysAgo: 30 },
    { projectIdx: 2, desc: 'Content migration from old site', minutes: 90, daysAgo: 25 },
    { projectIdx: 2, desc: 'QA testing and bug fixes', minutes: 120, daysAgo: 15 },
    { projectIdx: 2, desc: 'Production deployment and DNS cutover', minutes: 60, daysAgo: 12 },
  ];
  ...
  await db.insert(timeEntries).values(
    timeEntryDefs.map(t => ({
      ...
      billable: (t as any).billable ?? true,
    }))
  );

  // ── Invoices ─────────────────────────────────────────────────
  const invoiceDefs = [
    { projectIdx: 0, invoiceNumber: 'BK-2026-D01', amountCents: 2160000, status: 'paid' as const, dueOffset: -25, lineItems: [{ description: 'Discovery & Architecture Phase', quantity: 1, unitPrice: 14400, amount: 14400 }, { description: 'Project management (20 hours)', quantity: 20, unitPrice: 180, amount: 3600 }, { description: 'Infrastructure setup', quantity: 1, unitPrice: 3600, amount: 3600 }], paidDaysAgo: 22 },
    { projectIdx: 0, invoiceNumber: 'BK-2026-D02', amountCents: 2880000, status: 'sent' as const, dueOffset: 5, lineItems: [{ description: 'Development Phase - Sprint 1 & 2', quantity: 1, unitPrice: 21600, amount: 21600 }, { description: 'API integration development', quantity: 1, unitPrice: 7200, amount: 7200 }] },
    { projectIdx: 1, invoiceNumber: 'BK-2026-D03', amountCents: 325000, status: 'paid' as const, dueOffset: -15, lineItems: [{ description: 'Website Design - Discovery & Wireframes', quantity: 1, unitPrice: 2500, amount: 2500 }, { description: 'Brand consultation', quantity: 1, unitPrice: 750, amount: 750 }], paidDaysAgo: 12 },
    { projectIdx: 1, invoiceNumber: 'BK-2026-D04', amountCents: 325000, status: 'draft' as const, dueOffset: 15, lineItems: [{ description: 'Website Design - Final Mockups & Revisions', quantity: 1, unitPrice: 2500, amount: 2500 }, { description: 'Mobile responsive design', quantity: 1, unitPrice: 750, amount: 750 }] },
    { projectIdx: 2, invoiceNumber: 'BK-2025-D18', amountCents: 490000, status: 'paid' as const, dueOffset: -40, lineItems: [{ description: 'Full website build - 12 pages', quantity: 1, unitPrice: 3800, amount: 3800 }, { description: 'SEO optimization package', quantity: 1, unitPrice: 1100, amount: 1100 }], paidDaysAgo: 38 },
    { projectIdx: 2, invoiceNumber: 'BK-2025-D19', amountCents: 490000, status: 'paid' as const, dueOffset: -15, lineItems: [{ description: 'Final payment - launch & deployment', quantity: 1, unitPrice: 3900, amount: 3900 }, { description: 'Post-launch support (1 month)', quantity: 1, unitPrice: 1000, amount: 1000 }], paidDaysAgo: 10 },
  ];
  ...
  if (emailSendEntries.length > 0) {
    await db.insert(emailSends).values(
      emailSendEntries.map(e => ({
        ...e, sentBy: userId, status: 'sent' as const,
      }))
    );
  }
  ...
  const enrollmentDefs: Array<{ dealIdx: number; contactIdx: number; seqId: string; step: number; status: 'active' | 'paused' | 'completed' }> = [];

  if (constructionSeq) {
    enrollmentDefs.push(
      { dealIdx: 4, contactIdx: 7, seqId: constructionSeq.id, step: 2, status: 'active' },
      { dealIdx: 5, contactIdx: 8, seqId: constructionSeq.id, step: 1, status: 'active' },
      { dealIdx: 6, contactIdx: 9, seqId: constructionSeq.id, step: 1, status: 'active' },
      { dealIdx: 0, contactIdx: 0, seqId: constructionSeq.id, step: 3, status: 'completed' },
    );
  }
  if (localSeq) {
    enrollmentDefs.push(
      { dealIdx: 10, contactIdx: 13, seqId: localSeq.id, step: 1, status: 'active' },
      { dealIdx: 14, contactIdx: 18, seqId: localSeq.id, step: 1, status: 'active' },
      { dealIdx: 8, contactIdx: 11, seqId: localSeq.id, step: 2, status: 'paused' },
    );
  }
  ...
  const notificationDefs = [
    ...
    { type: 'task_due' as const, title: 'Task Due Tomorrow', body: 'Implement maintenance scheduling module — Crossland Heavy project', entityType: null, entityIdx: null, isRead: false, daysAgo: 0 },
    ...
    { type: 'task_due' as const, title: 'Task Due: Design homepage', body: 'NTX Dental — Design homepage and service pages is due in 2 days.', entityType: null, entityIdx: null, isRead: false, daysAgo: 0 },
  ];
