...
export default function StatCard({ label, value, trend, trendColor = 'gray', hero = false }: StatCardProps) {
  ...
  if (hero) {
    return (
      <div className="rounded-lg bg-surface border border-border border-l-4 border-l-[#1F4D78] p-7 flex flex-col justify-center h-full">
        <p className="text-xs font-medium uppercase tracking-wider text-gray-500">{label}</p>
        <p className="mt-3 text-4xl font-bold text-gray-100">{value}</p>
        {trend && (
