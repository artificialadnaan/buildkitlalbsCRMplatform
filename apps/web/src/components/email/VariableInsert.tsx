  ...
  return (
    <div className="relative">
      <button
        ...
        className="bg-surface border border-border rounded-md px-3 py-1.5 text-xs text-gray-400 hover:text-gray-200 hover:border-gray-500"
      >
        {'{{ Insert Variable }}'}
      </button>
        ...
        <div className="absolute top-full left-0 mt-1 bg-surface border border-border rounded-md shadow-lg z-10 w-56">
          {VARIABLES.map(v => (
            <button
              ...
              className="w-full text-left px-3 py-2 text-sm text-gray-300 hover:bg-white/5 border-b border-border last:border-0"
            >
              <span className="text-gray-200">{v.label}</span>
