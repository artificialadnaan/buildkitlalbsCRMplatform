...
export default function ProjectDetail() {
  ...
  const loadProject = useCallback(() => {
    ...
    api<Milestone[]>(`/api/projects/${id}/milestones`).then(data => {
      setMilestones(data);
      // Auto-select first in-progress or first pending milestone
  ...
  // Load tasks when active milestone changes
  useEffect(() => {
  ...
  async function handleToggleTaskStatus(taskId: string, newStatus: string) {
    ...
    // Reload tasks and milestones (milestone may auto-complete)
    api<Task[]>(`/api/milestones/${activeMilestoneId}/tasks`).then(setTasks);
    ...
    <div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        <div className="bg-surface border border-border rounded-lg p-4">
          ...
          <div className="mt-1">
            <Badge label={project.project.status} variant={statusVariant[project.project.status]} />
          </div>
        </div>
        <div className="bg-surface border border-border rounded-lg p-4">
        ...
        <div className="bg-surface border border-border rounded-lg p-4">
          ...
          <div className="text-lg font-bold text-gray-200 mt-1">
            {project.project.budget != null ? `$${project.project.budget.toLocaleString()}` : '---'}
          </div>
        </div>
        <div className="bg-surface border border-border rounded-lg p-4">
          <div className="text-xs uppercase text-gray-500">Time Logged</div>
          <div className="text-lg font-bold text-gray-200 mt-1">
            {timeSummary ? formatHours(timeSummary.totalMinutes) : '0h'}
          </div>
          {timeSummary && timeSummary.totalMinutes > 0 && (
            <div className="text-xs text-gray-500 mt-0.5">
              {formatHours(timeSummary.billableMinutes)} billable
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-surface border border-border rounded-lg p-4 mb-6">
        <ProgressBar
          ...
          label={`${doneMilestones} of ${milestones.length} milestones complete`}
          color={milestoneProgress === 100 ? 'bg-green-500' : 'bg-blue-500'}
        />
      </div>

      {/* Two-column layout: Milestones + Tasks */}
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-surface border border-border rounded-lg p-4">
          ...
          <MilestoneTimeline
            ...
            activeMilestoneId={activeMilestoneId || undefined}
            onMilestoneClick={m => setActiveMilestoneId(m.id)}
          />
        </div>
        ...
        <div className="bg-surface border border-border rounded-lg p-4 col-span-2">
            ...
            <h3 className="font-semibold text-gray-200">
              ...
              {activeMilestoneId && (
                <span className="text-gray-500 font-normal text-sm ml-2">
                  {milestones.find(m => m.id === activeMilestoneId)?.name}
                </span>
              )}
            </h3>
            {activeMilestoneId && (
          ...
          {!activeMilestoneId ? (
            <p className="text-sm text-gray-600 py-4 text-center">Select a milestone to view its tasks</p>
          ) : (
            <TaskList
              tasks={tasks}
      ...
      <div className="bg-surface border border-border rounded-lg p-4">
        ...
        <div className="border-t border-border">
          {timeEntries.length === 0 ? (
            <p className="text-sm text-gray-600 py-4 text-center">No time logged yet</p>
          ) : (
                ...
                {timeEntries.slice(0, 10).map(entry => (
                  <tr key={entry.id} className="border-b border-border last:border-0">
                    ...
                    <td className="px-3 py-2 text-right">
                      <Badge label={entry.billable ? 'Yes' : 'No'} variant={entry.billable ? 'green' : 'gray'} />
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <Modal open={showAddTask} onClose={() => setShowAddTask(false)} title="Add Task">
        <div className="space-y-3">
          ...
          <button
            ...
            className="w-full bg-blue-600 text-white rounded-md py-2 text-sm font-medium hover:bg-blue-500"
          >
            Add Task
          </button>
        </div>
      ...
      <Modal open={showLogTime} onClose={() => setShowLogTime(false)} title="Log Time">
        <TimeEntryForm
          ...
          onCancel={() => setShowLogTime(false)}
          tasks={tasks.filter(t => t.status !== 'done').map(t => ({ id: t.id, title: t.title }))}
        />
      </Modal>
    </div>
  );
