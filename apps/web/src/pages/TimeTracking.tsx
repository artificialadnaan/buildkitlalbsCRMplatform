  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <StatCard
          label="Total Hours"
          value={summary ? formatHours(summary.totalMinutes) : '0h'}
        />
        <StatCard
          label="Billable Hours"
          value={summary ? formatHours(summary.billableMinutes) : '0h'}
          trendColor="text-green-500"
          trend={summary && summary.totalMinutes > 0
            ? `${Math.round((summary.billableMinutes / summary.totalMinutes) * 100)}% of total`
            : undefined}
        />
        <StatCard
          label="Non-Billable Hours"
          value={summary ? formatHours(summary.nonBillableMinutes) : '0h'}
        />
      </div>
                ...
                {summary.byProject.map(p => (
                  <div key={p.projectId} className="flex justify-between items-center text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-gray-400">{formatHours(p.totalMinutes)}</span>
                      <span className="text-xs text-gray-600">
                        ({formatHours(p.billableMinutes)} billable)
                      </span>
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-2 pt-3">
                {summary.byUser.map(u => (
                  <div key={u.userId} className="flex justify-between items-center text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-gray-400">{formatHours(u.totalMinutes)}</span>
                      <span className="text-xs text-gray-600">
                        ({formatHours(u.billableMinutes)} billable)
                      </span>
                    </div>
                  </div>
                ))}
